...
const getAccessToRoute=(req,res,next)=>{
    ...
    jwt.verify(access_token,JWT_SECRET_KEY,(err,decoded)=>{
        if(err){
            return next(new CustomError("You are not authorized to accesss this route",401))
        }
        req.user={
            id:decoded.id,
            name:decoded.name
...
module.exports={getAccessToRoute,getAdminAccess,
    getQuestionOwnerAccess,getAnswerOwnerAccess,tokenUser};
